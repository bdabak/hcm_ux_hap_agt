    ...
    return BaseController.extend("hcm.ux.hapv3.controller.FormList", {
      ...
      onInit: function () {
        ...
        oViewModel = new JSONModel({
          ...
          periodSelection: oThis._initiatePeriods(),
          selectedDates: (function () {
            return [new Date().getFullYear(), new Date().getFullYear() + 1];
          })(),
          selectedPeriods: (function () {
            var m = moment(new Date()).hour(9);
            var cY = m.format("YYYY");
            var cQ = m.format("Q");
            return [cY + "-" + cQ];
          })(),
          viewBusy: false,
      ...
      onRefreshFormList: function (oEvent) {
        ...
        var aSelectedPeriods = oViewModel.getProperty("/selectedPeriods");
        // var aDates = oViewModel.getProperty("/dateSelection");
        var aPeriods = oViewModel.getProperty("/periodSelection");
        ...
        var _getPeriodDate = function (sPeriod) {
          var oPeriod = _.find(aPeriods, ["Period", sPeriod]);
          if (oPeriod) {
            return [oPeriod.Begda, oPeriod.Endda];
          } else {
            return [];
          }
        };
        ...
        $.each(aRoles, function (i, sRole) {
          $.each(aSelectedPeriods, function (j, sPeriod) {
            var aDate = _getPeriodDate(sPeriod);
            var oUrlParam = {
              StartDate: aDate[0],
              EndDate: aDate[1],
              RoleId: sRole,
        ...
        this.openBusyFragment("formListPrepared", []);
      ...
      _initiatePeriods: function () {
        ...
        while (cY>=bY) {
          while(cQ >= 1 ){
            var q = moment(cY+"/01/01", "YYYY/MM/DD").quarter(cQ);
            var oDate = {
              Period: cY + "-" + cQ,
              PeriodText: this.getText("periodItem", [cY, cQ]),
              Begda: q.startOf("quarter").toDate().setHours(9),
              Endda: q.endOf("quarter").toDate().setHours(9)
            };
            aDates.push(oDate);
            cQ--;
          }
